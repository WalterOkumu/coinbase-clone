...
const Prices = () => {
  ...
  const listTab = [{ status: 'All Assets' }, { status: 'Gainers' }, { status: 'Losers' }]
        ...
        <View style={{ paddingHorizontal: 20, paddingTop: 50, marginBottom: 90 }}>
          <View style={{ paddingTop: 50 }}>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
              <Text style={{ color: '#090c0d', paddingTop: 5, fontSize: 29, fontWeight: 'bold' }}>
                MARKET IS DOWN 70%
              </Text>
              <View
                style={{
            ...
            <View style={styles.listTab}>
              {
                listTab.map((list, index) => (
                  <TouchableOpacity
                    key={index}
                    style={[styles.btnTab, status === list.status && styles.btnTabActive]}
                    onPress={(event) => setStatusFilter(list.status)}
                  >
                ...
                <View key={index}>
                  ...
                  >
                    ...
                    <View style={{ paddingLeft: 15 }}>
                      ...
                      <Text style={{ fontSize: 14, fontWeight: '300', color: '#5d616d' }}>0 {coin.symbol.toUpperCase()}</Text>
                    </View>
                  </View>
                </View>
              ))
